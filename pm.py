...
def inputAndValidateMasterPassword():
    ...
    if hashed_mp != result[0][0].masterkey_hash:
        printc("[red][!] WRONG! [/red]")
        return None

    return [mp,result[0][0].device_secret]
...
def main():
    ...
    if args.option in ["extract","e"]:
        ...
        if res is not None:
            utils.retrieve.retrieveEntries(res[0],res[1],search["siteurl"],decryptPassword = args.copy)
